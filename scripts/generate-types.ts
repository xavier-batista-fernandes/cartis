...
const districtNames = data.objects.municipalities.geometries.map(
	(item: any) => item.properties.NAME_1,
);
...
const municipalityNames = data.objects.municipalities.geometries.map(
	(item: any) => item.properties.NAME_2,
);
